...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className="bg-[#1a2a40] text-white shadow-md sticky top-0 z-50 border-b-4 border-[#c5a059]">
        ...
        <div className="flex justify-between h-20 items-center">
            ...
            {navItems.map((item) => (
             ...
             {secondaryItems.map((item) => (
              <Link
                key={item.path}
                to={item.path}
                className={`p-2.5 rounded-md transition-all duration-200 text-white/70 hover:text-[#1a2a40] hover:bg-[#c5a059] focus-visible:ring-2 focus-visible:ring-[#c5a059] focus:outline-none`}
                title={item.label}
          ...
          <div className="lg:hidden flex items-center">
            <button
              ...
              className="text-white hover:text-[#c5a059] p-2 focus-visible:ring-2 focus-visible:ring-[#c5a059] focus:outline-none focus:rounded-sm"
      ...
      {isOpen && (
        <div id="mobile-menu-dropdown" className="lg:hidden bg-[#1a2a40] border-t border-white/10 animate-in slide-in-from-top-2 duration-200">
