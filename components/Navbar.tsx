    ...
    <nav className="bg-[#1a2a40] text-white shadow-md sticky top-0 z-50 border-b-4 border-[#c5a059]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-20 items-center">
          {/* Logo / Brand */}
          <Link to="/" className="flex-shrink-0 flex items-center gap-4 group focus-visible:ring-2 focus-visible:ring-[#c5a059] focus-visible:outline-none rounded">
             <div className="w-10 h-10 bg-[#c5a059] flex items-center justify-center rounded-sm shadow-sm group-hover:scale-105 transition-transform" aria-hidden="true">
          ...
          <div className="hidden lg:flex items-center space-x-2">
            {navItems.map((item) => (
              <Link
                key={item.path}
                to={item.path}
                className={`px-4 py-2 rounded-md text-[11px] font-bold uppercase tracking-widest transition-all duration-200 flex items-center gap-2 focus-visible:ring-2 focus-visible:ring-[#c5a059] focus-visible:outline-none
             ...
             {secondaryItems.map((item) => (
              <Link
                key={item.path}
                to={item.path}
                className={`p-2.5 rounded-md transition-all duration-200 text-white/70 hover:text-[#1a2a40] hover:bg-[#c5a059] focus-visible:ring-2 focus-visible:ring-[#c5a059] focus-visible:outline-none`}
                title={item.label}
          ...
          <div className="lg:hidden flex items-center">
            <button
              ...
              className="text-white hover:text-[#c5a059] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#c5a059] rounded p-2"
          ...
          <div className="px-2 pt-4 pb-6 space-y-2 sm:px-3">
            {navItems.map((item) => (
              <Link
                ...
                className={`block px-4 py-3 rounded-md text-xs font-bold uppercase tracking-widest flex items-center gap-4 focus-visible:ring-2 focus-visible:ring-[#c5a059] focus-visible:outline-none
            ...
            <div className="border-t border-white/10 my-4 pt-4 px-2">
               ...
               {secondaryItems.map((item) => (
                  <Link
                    ...
                    to={item.path}
                    onClick={() => setIsOpen(false)}
                    className="flex flex-col items-center justify-center p-3 rounded-md text-white/80 hover:bg-white/10 hover:text-[#c5a059] gap-2 bg-[#131f30] focus-visible:ring-2 focus-visible:ring-[#c5a059] focus-visible:outline-none"
                  >
