    ...
    <div className="p-6 sm:p-12 lg:p-24 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col xl:flex-row gap-8 lg:gap-12">
        <div className="flex-1 bg-white border-2 sm:border-4 border-[#1a2a40] p-6 sm:p-10 shadow-lg">
            ...
            {success && (
              <div role="alert" aria-live="polite" className="bg-emerald-50 border border-emerald-200 text-emerald-700 px-4 py-3 mb-6 mono text-xs font-bold uppercase flex items-center gap-2">
                <CheckCircle size={16} /> Request Submitted Successfully
              </div>
            )}

            {error && (
              <div role="alert" aria-live="polite" className="bg-rose-50 border border-rose-200 text-rose-700 px-4 py-3 mb-6 mono text-xs font-bold uppercase">
                {error}
              </div>
            )}
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-[#1a2a40] text-white font-black py-4 uppercase tracking-[0.2em] sm:tracking-[0.3em] hover:bg-[#c5a059] hover:text-[#1a2a40] transition-all shadow-md active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-3"
                >
                    {loading ? (
                      <>
